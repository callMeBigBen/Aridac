...
# 3. decrease a fio docker B to 200KB/s
...
sudo docker run -dit --rm --device-write-bps /dev/sda:1000000mb --device-read-bps /dev/sda:1000000mb --mount src=/mnt,target=/test_container,type=bind fio_test:v3 sh -c "./incdec_disk.sh" &