...
def selector(containers, name_to_ratio_i, name_to_ratio_o):
    ...
    for c in containers:
        ...
        if len(name_to_ratio_i[c]) > 1:
            desired_in[c] = MAX_DISK_IPS * \
                pow(1024, 2) * sum(name_to_ratio_i[c])/len(name_to_ratio_i[c])

            if desired_in[c] * (1+ELASTIC) == limit_in[c]:
                del desired_in[c]

        if len(name_to_ratio_o[c]) > 1:
            desired_out[c] = MAX_DISK_OPS * 1024 * 1024 * \
                sum(name_to_ratio_o[c])/len(name_to_ratio_o[c])
    ...
    for c, desire in desired_in.items():
        if desired_in[c] * (1+ELASTIC) > limit_in[c] and idle_ips > 0:
            ...
            limit_in[c] = min(desired_in[c] * (1+ELASTIC),
                              desired_in[c] + idle_ips)
    ...
    for c, desire in desired_out.items():
        if desired_out[c] * (1+ELASTIC) > limit_out[c] and idle_ops > 0:
            ...
            limit_out[c] = min(desired_out[c] * (1+ELASTIC),
                               desired_out[c] + idle_ops)
    ...
    with open('limit_repeating.csv', 'a', newline='') as csvfile:
        ...
        for c in containers:
            spamwriter.writerow(
                [str(current_time)] + [c] + [str(limit_in[c]/1024)] + [str(limit_out[c]/1024)])
